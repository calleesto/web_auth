...
Projekt realizowany w ramach przedmiotu Cyberbezpieczeństwo. Celem jest demonstracja różnych mechanizmów kontroli dostępu w nowoczesnym API webowym.
...
* **Język:** C# 12 / .NET 8 (silne typowanie, bezpieczeństwo pamięci).
...
W ramach analizy teoretycznej wyróżniliśmy cztery główne modele:
...
2. **ABAC (Attribute-Based Access Control):** Dostęp oparty na atrybutach (kontekstowy). Decyzja zapada na podstawie cech użytkownika, czasu lub miejsca (np. "tylko w godzinach pracy").
...
**W projekcie zaimplementowano modele RBAC oraz ABAC.**
